...
func TestACLEndpoint_TokenSet(t *testing.T) {
	...
	t.Run("Create it using Roles linked by id and name", func(t *testing.T) {
		...
		require.Equal(t, role1.ID, token.Roles[0].ID)
		require.Equal(t, role2.ID, token.Roles[1].ID)
		// names are returned because they are denormalized on write
		require.Equal(t, role1.Name, token.Roles[0].Name)
		require.Equal(t, role2.Name, token.Roles[1].Name)
	})
...
func TestACLEndpoint_RoleSet(t *testing.T) {
	...
	t.Run("Create it", func(t *testing.T) {
		...
		require.Equal(t, fakePolicyID_A, role.Policies[0].ID)
	...
	t.Run("Update it", func(t *testing.T) {
		...
		require.Len(t, role.Policies, 1)
		require.Equal(t, fakePolicyID_B, role.Policies[0].ID)
	})
}
